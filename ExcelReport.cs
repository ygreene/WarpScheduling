            ...
            using (SpreadsheetDocument document = SpreadsheetDocument.Create(@"C:\STIRoot\" +docname , SpreadsheetDocumentType.Workbook))
            {
                ...
                foreach (var i in wp.Where(w=> w.WarperID ==v.warperid).Take(1) )
                {
                    ...
                    sheets.Append(sheet);
                        ...
                        columns.Append(new Column() { Min = 1, Max = 6, Width = 30, CustomWidth = true });

                    
                        SheetData sheetdata = new SheetData();
                        sheetdata.AppendChild(columns);
                        worksheetpart.Worksheet = new Worksheet(sheetdata);
                    
                        //  worksheetpart.Worksheet.Append(columns);

                        Row TitleRow = new Row();
                        TitleRow.Append(new Cell() { DataType = CellValues.String, CellValue = new CellValue("Warping Prioirty"), StyleIndex=3 });
                        ...
                        headerRow.Append(new Cell() { DataType = CellValues.String, CellValue = new CellValue("Prioirty") });
                        headerRow.Append(new Cell() { DataType = CellValues.String, CellValue = new CellValue("Warp MO") });
                        headerRow.Append(new Cell() { DataType = CellValues.String, CellValue = new CellValue("Warp Style") });
                        headerRow.Append(new Cell() { DataType = CellValues.String, CellValue = new CellValue("Tickets") });
                        headerRow.Append(new Cell() { DataType = CellValues.String, CellValue = new CellValue("Due Date") });
                        ...
                        foreach (var z in wp.Where(w => w.WarperID == v.warperid).OrderBy(w => w.Priority))
                        {
                            ...
                            mrow.Append(new Cell() { DataType = CellValues.Number, CellValue = new CellValue(z.Priority.ToString()) });
                            mrow.Append(new Cell() { DataType = CellValues.String, CellValue = new CellValue(z.WarpMO) });
                            mrow.Append(new Cell() { DataType = CellValues.String, CellValue = new CellValue(z.WarpStyle) });
                            mrow.Append(new Cell() { DataType = CellValues.Number, CellValue = new CellValue(z.TotalTickets.ToString()) });
                           mrow.Append(new Cell() { DataType = CellValues.String, CellValue = new CellValue(z.EarliestDueDate.ToShortDateString()) });
